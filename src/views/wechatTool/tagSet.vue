<template>
  <div class="session-list">
    
    <div class="title clearfix">
      <title-item class="title-left" name="标签列表" fontSize="16px"></title-item>
      <div class="el-button-box">
        <el-button icon="el-icon-upload" size="mini" @click="UploadExcel">批量上传</el-button>
        <el-button type="success" size="mini" @click="addTagLog">添加</el-button>
      </div>
    </div>
    <page-data :data="pageList" style="padding: 20px 10px">
      <template slot="table">
        <el-table
          :data="pageList.records"
          :loading="loading"
          :height="($store.state.app.viewHeight - 146)"
          border
          style="width: 100%">
          <el-table-column
            type="index"
            width="50"
            label="序号">
          </el-table-column>
          <el-table-column
            prop="answer"
            label="标签名"
            :show-overflow-tooltip="true">
            <template  slot-scope="scope">
              <el-tag>{{scope.row.answer}}</el-tag>
            </template>
          </el-table-column>
          <el-table-column
            prop="question"
            label="问题"
            :show-overflow-tooltip="true">
          </el-table-column>
          <el-table-column
            prop="synonymList"
            label="相似问题"
            :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <el-popover v-if="scope.row.synonymList.length > 0" placement="bottom" trigger="hover">
                <ul style="margin-left: 15px;">
                  <li style="list-style-type: none" v-for="(item,index) in scope.row.synonymList" :key="item.id">
                    {{index + 1}}. {{item.content}}
                  </li>
                </ul>
                <span slot="reference" v-if="scope.row.synonymList.length > 0">1. {{scope.row.synonymList[0].content}}</span>
              </el-popover>
            </template>
          </el-table-column>
          <!-- <el-table-column
            prop="timeSlot"
            label="时间范围"
            :show-overflow-tooltip="true">
          </el-table-column> -->
          <el-table-column
            prop="createDate"
            label="创建时间"
            :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <span>{{scope.row.createDate | format('yyyy-MM-dd hh:mm') }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="updateDate"
            label="更新时间"
            :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <span>{{scope.row.updateDate | format('yyyy-MM-dd hh:mm') }}</span>
            </template>
          </el-table-column>
          <el-table-column
            fixed="right"
            prop="done"
            label="操作"
            width="240">
            <template slot-scope="scope">
              <el-button-group>
                <el-button size="mini" type="primary" icon="el-icon-view" @click="viewUser(scope.row)">查看</el-button>
                <el-button size="mini" type="primary" icon="el-icon-edit" @click="editTag(scope.row)">编辑</el-button>
                <el-button size="mini" type="danger" icon="el-icon-delete" @click="moveTag(scope.row)">删除</el-button>
              </el-button-group>
            </template>
          </el-table-column>
        </el-table>
      </template>
    </page-data>
    <el-dialog
      title="标签设置"
      :visible.sync="showTag"
      width="800px">
      <addTag :edit="edit" @close="closeAddLog" v-if="showTag"></addTag>
    </el-dialog>
    <el-dialog
      title="标签用户"
      :visible.sync="showTagUser"
      width="1000px">
      <tagUser @close="closeAddLog" v-if="showTagUser"></tagUser>
    </el-dialog>
    <el-dialog title="批量上传" :visible.sync="isUpload" width="800px">
      <uploadMsgExcel v-if="isUpload" type="3" :templateLink="`https://bj-bdy-public.oss-cn-beijing.aliyuncs.com/online/upload/%E6%A0%87%E7%AD%BE%E8%AE%BE%E7%BD%AE.xls`" @close="closeAddLog"></uploadMsgExcel>
    </el-dialog>
  </div>
</template>
<script lang="ts">
import { Component, Vue } from 'vue-property-decorator'
import addTag from './components/addTag.vue'
import tagUser from './components/tagUser.vue'
import uploadMsgExcel from './components/uploadMsgExcel.vue'
import miniPage from '../../mixin/MixinPage'

@Component({
  components: {
    addTag,
    uploadMsgExcel,
    tagUser
  },
  mixins: [miniPage]
})
export default class SessionSet extends Vue {
  private tableData: any[] = []
  private loading: boolean = true
  private showTag: boolean = false
  private showTagUser: boolean = false
  private edit: boolean = false
  private pageTotal: number = 1
  private isUpload: boolean = false
  private pageOptions: any = {
    size: 10,
    current: 1
  }
  private pageList: any = {
    current: 1,
    size: 10,
    total: 0,
    records: []
  }
  public handleGetList: any
  // SessionSet
  private get params () {
    return {
      cid: this.$store.state.user.userInfo.cid,
      gid: this.$store.state.user.userInfo.gid
    }
  }
  private async created () {
    await this.handleGetList()
  }
  private async getList (params?: any, form?: any) {
    this.loading = true
    if (this.params.cid) {
      const data = await this.$store.dispatch('wxtool/getSingleList', {
        ...this.params,
        type: 3,
        ...params,
        ...form
      })
      this.pageList = data
      this.loading = false
    } else {
      await this.$store.dispatch('user/getUserInfo')
      this.getList()
    }
  }
  private UploadExcel () {
    this.isUpload = true
  }
  private closeAddLog () {
    this.showTag = false
    this.showTagUser = false
    this.getList()
  }
  private viewUser (item: any) {
    this.$store.commit('wxtool/SET_SINGLETAGID', item.id)
    this.showTagUser = true
  }
  private async moveTag (item: any) {
    this.$confirm('此操作将永久删除该标签, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      const data = await this.$store.dispatch('wxtool/wechatDeleteSingle', item.id)
      if (data.errcode === 200) {
        this.$notify({
          title: '提示',
          message: `${item.answer}标签，已删除`,
          type: 'success'
        })
        this.getList()
      } else {
        this.$notify({
          title: '提示',
          message: `${item.answer}标签，因${data.data}删除失败`,
          type: 'error'
        })
      }
    })
  }
  private editTag (item: any) {
    this.$store.commit('wxtool/SET_SINGLETAGID', item.id)
    this.edit = item.id
    this.showTag = true
  }
  private addTagLog () {
    this.edit = false
    this.showTag = true
  }
}
</script>
<style lang="scss" scoped>
.session-list {
  font-size: 14px;
  .title {
    display: flex;
    padding: 20px 0;
    text-align: right;
    justify-content: space-between;
    .title-left {
      flex: 1;
    }
    .el-button {
      // width: 200px;
      margin-right: 20px;
    }
  }
}
</style>

